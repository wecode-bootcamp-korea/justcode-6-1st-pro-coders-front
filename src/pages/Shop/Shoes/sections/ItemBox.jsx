				...
				<NavLink to={`/product/${item.id - 1}`} className='item-a-box'>
					...
					<div className='item-content'>
						...
						<div className='item-price-box'>
							{/* 할인 유/무 여부 */}
							<div>
								{item.is_discounted === 1 ? <p className='item-price-sale-on'>{item.price}원</p> : <p className='item-price-sale-off'>{item.price}원</p>}
							</div>
							{/* 정가 */}
    ...
    <>
      ...
      {item && (
        <Link to={`/product/${item.id - 1}`} className='item-a-box'>
          ...
          {/* 상품 분류, 이름, 가격 */}
          <div className='item-content'>
            <p className='item-cate'>{item.keyword}</p>{' '}
            {/* 상품분류 [신상/할인 ...] */}
            <p className='item-title'>{item.title}</p> {/* 상품이름 */}
            {/* 상품가격박스 */}
            <div className='item-price-box'>
              ...
              <div>
                {item.is_discounted === 1 ? (
                  <p className='item-price-sale-on'>{item.price}원</p>
                ) : (
                  <p className='item-price-sale-off'>{item.price}원</p>
                )}
              </div>
              {/* 정가 */}
              {item.is_discounted === 1 ? (
                <p className='item-sale-box'>
                  <span className='item-sale-percent'>
