        ...
        <a href='' className='item-a-box'>
          ...
          <div className='item-content'>
            ...
            <div className='item-price-box'>
              {/* 할인 유/무 여부 */}
              <div>{productdata.data[index].is_discounted === 1 ? <p className='item-price-sale-on'>{productdata.data[index].price}</p> : <p className='item-price-sale-off'>{productdata.data[index].price}</p>}</div>
              {/* 정가 */}
              <p className='item-sale-box'>
                <span className='item-sale-percent'>{productdata.data[index].discount_percent}</span> {/* 할인 % */}
                <span className='item-sale-price'>{productdata.data[index].discounted_price}</span> {/* 할인된 가격 */}
              </p>
            </div>
            {/* 착용가능 성별 및 쿠폰여부 */}
